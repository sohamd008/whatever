...
const convertValue = ({ category, value, fromUnit, toUnit }) => {
  if (category === 'Temp') {
    let baseC = value;
  ...
  return finalValue.toLocaleString(undefined, { maximumFractionDigits: 4 });
};
...
const UnitConverter = () => {
  ...
  const handleCategoryChange = (nextCategory) => {
    const keys = Object.keys(UNITS[nextCategory]);
  ...
  return (
    <div className="p-6 glass-card h-full flex flex-col min-h-[380px]">
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            >
              {Object.keys(UNITS[category]).map((unit) => <option key={unit} value={unit}>{unit}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <select
