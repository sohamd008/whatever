      ...
      <div className="px-6 py-4 bg-black/60 border-b border-white/10 flex justify-between items-center z-40">
        <div className="flex items-center gap-4">
             <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-full border border-white/5 shadow-inner">
                ...
                <span className="text-[9px] font-mono tracking-widest text-muted">{isOnline ? 'LOCAL LINK' : 'OFFLINE MODE'}</span>
             </div>
        ...
        <div className="flex gap-2">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className={`p-2 rounded-2xl transition-all ${isSidebarOpen ? 'bg-primary/20 text-primary shadow-gold-glow' : 'bg-black/40 text-muted hover:text-white border border-white/5'}`}>
                <List className="w-4 h-4" />
            </button>
            <button onClick={() => setIsPreview(!isPreview)} className={`p-2 rounded-2xl transition-all ${isPreview ? 'bg-tertiary/20 text-tertiary shadow-cyan-glow' : 'bg-black/40 text-muted hover:text-white border border-white/5'}`}>
      ...
      <div className="flex flex-grow relative overflow-hidden">
        ...
        <div className={`absolute inset-y-0 left-0 w-80 bg-background/98 backdrop-blur-2xl border-r border-white/10 z-50 transition-all duration-500 ease-in-out transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} p-6 flex flex-col`}>
            <div className="flex justify-between items-center mb-8">
                <h4 className="text-[10px] font-mono font-bold tracking-[0.3em] text-muted/60 uppercase">System Segments</h4>
            ...
            <button onClick={createNote} className="w-full h-14 bg-primary text-black rounded-[1.25rem] font-heading font-bold flex items-center justify-center gap-3 mb-6 hover:scale-[1.02] active:scale-95 transition-all shadow-gold-glow">
                <Plus className="w-5 h-5 stroke-[3px]" /> CREATE NEW
            </button>
